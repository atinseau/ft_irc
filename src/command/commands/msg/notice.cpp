...
void Command::notice(Payload &p)
{
	...
	if (p.body.second.size() < 2)
		throw ResponseException(ERR_NEEDMOREPARAMS(client.get_key("NICKNAME"), "NOTICE"));
	...
	for (std::vector<Recipient>::iterator it = recipients.begin(); it != recipients.end(); it++)
	{
		if (it->is_channel)
		{
			...
			dispatcher.load(RPL_NOTICE(client.fullname(), it->identifier, message));
		...
		else
		{
			...
			if (recipient == NULL)
				...
				recipient->write(RPL_NOTICE(client.fullname(), it->identifier, message));
		}
	}
}